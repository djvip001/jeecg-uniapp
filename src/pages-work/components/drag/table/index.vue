...
function initOption (data){
  ...
  if (config.dataType == '2' || config.dataType == 2) {
    ...
    const allCol = cloneDeep(config?.option?.columns)
    // 获取要展示的列
    const showCol = allCol.filter((item) => item.izShow === 'Y')
    // 判断是否显示操作列
    ...
    columns.value = showCol.map((item) => {
      item.width = showCol.length < 3 ? '50%' : '150px'
      item.ellipsis = true
      return item
    })
  } else {
