        ...
        <wd-form ref="form" :model="formData">
          <wd-cell-group class="mb-14px" border>
            ...
            >
              ...
              >
                <view style="text-align: left">
                  <wd-switch
                    ...
                    :active-value="switchOpt(item.formSchema?.extendOption || item.formSchema?.customValue, 0)"
                    :inactive-value="switchOpt(item.formSchema?.extendOption || item.formSchema?.customValue, 1)"
                  />
                </view>
              </wd-cell>
              ...
              <CustomInput
                v-else-if="
                  item.type === 'inputNumber' && (!item.onlyInteger || item.onlyInteger == false)
                "
              ...
              <!-- 数字框 整数 -->
              <CustomInput
                v-else-if="item.type === 'inputNumber' && item.onlyInteger === true"
                :label-width="labelWidth"
                :label="get4Label(item.label)"
                :name="item.key"
                v-model:value="formData[item.key]"
              ...
              <PopupDict
                ...
                @selected="popupDictChange($event, item)"
              ></PopupDict>
              <!-- popup -->
              <Popup
...
const linkRecordChange = (linkRecord, key) => {
  // 关联记录字段自身的翻译文本，便于一对多表格回显
  const linkTableItem = rootProperties.value.find(
    (item) => item.key === key && item.type === 'link_table',
  )
  if (linkTableItem) {
    const titleField =
      linkTableItem.formSchema?.dictText && linkTableItem.formSchema.dictText.split(',')[0]
    if (titleField) {
      const showText = linkRecord
        .map((record) => record[titleField])
        .filter((v) => v != null && v !== '')
        .join(',')
      nextTick(() => {
        formData.value[key + '_dictText'] = showText
      })
    }
  }
...
/**
 * Popup 字典同步保存翻译文本，供一对多表格回显
 */
const popupDictChange = (rows, item) => {
  const labelField = item.formSchema?.destFields
  const showText = rows
    .map((record) => record[labelField])
    .filter((value) => value != null && value !== '')
    .join(',')
  nextTick(() => {
    formData.value[item.key + '_dictText'] = showText
  })
}
/**
 * 校验字段
...
function createRootProperties(formSchema: any) {
  ...
  properties.forEach((item) => {
    if (item?.key && item?.key != 'action') {
      ...
      const formSchema = { ...item, pattern: patternInfo?.pattern, errorInfo: patternInfo?.message }
      if (item.type == 'sel_tree') {
        // 自定义字典树
        const dictText = item.dictText.split(',')
        formSchema.dict = `${item.dictTable},${dictText[2]},${dictText[0]}`
        formSchema.hasChildField = dictText[3] ?? 'has_child'
        formSchema.pidField = dictText[1]
        formSchema.pidValue = item.dictCode ?? '0'
      } else if (item.type == 'link_table') {
        // 关联记录
        const ta_field = properties.filter(
          (_item) => _item.type === 'link_table_field' && _item.dictTable === item.key,
        )
        ta_field.forEach((taItem) => {
          if (!item.dictText.includes(taItem.dictText)) {
            formSchema.dictText += `,${taItem.dictText}`
          }
        })
      } else if (item.type == 'cat_tree') {
        // 分类字典树
        // update-begin-author:liaozhiyang date:2026-08-03 for:【LHZP-1716】修复online一对多有分类字典树组件报错
        formSchema.pidCode = item.dictCode ?? ''
        // update-end-author:liaozhiyang date:2026-08-03 for:【LHZP-1716】修复online一对多有分类字典树组件报错
      } else if (item.type === 'popup_dict') {
        formSchema.code = item.dictTable
        formSchema.destFields = item.dictText
        formSchema.orgFields = item.dictCode
      }
...
function handleDefaultValue() {
  rootProperties.value.forEach((item) => {
    ...
    loadOneFieldDefVal(defVal, type, (value) => {
      // 异步回调触发时，若已切换为编辑模式（beforeOpen 已调用），不覆盖编辑数据
      if (!isUpdate.value) {
