  ...
  <view class="onlineLoader-container">
    <scroll-view class="scrollArea" scroll-y>
      <view class="form-container">
        ...
        <wd-tabs
          v-if="!single && subTable && subTable.length > 0"
          custom-class="mt-14px"
          animated
          :slidable-num="3"
          autoLineWidth
        >
    ...
    <view class="footer" v-if="showFooter && showSubmitBtn">
      <wd-button :disabled="loading" block :loading="loading" @click="formSubmit">提交</wd-button>
    </view>
  </view>
  <wd-toast :selector="selectorId"></wd-toast>
</template>

<script lang="ts" setup>
import FormProperty from './FormProperty'
import OnlineImage from './view/online-image.vue'
import OnlineFile from './view/online-file.vue'
import OnlineFileCustom from './view/online-file-custom.vue'
import OnlineSelect from './view/online-select.vue'
import OnlineTime from './view/online-time.vue'
import OnlineDatetime from './view/online-datetime.vue'
import OnlineDate from './view/online-date.vue'
import OnlineRadio from './view/online-radio.vue'
import OnlineCheckbox from './view/online-checkbox.vue'
import OnlineMulti from './view/online-multi.vue'
import CustomInput from './view/CustomInput.vue'
import OnlinePca from './view/online-pca.vue'
import OnlinePopupLinkRecord from './view/online-popup-link-record.vue'
...
const props = defineProps({
  ...
  // update-begin-author:liaozhiyang date:2025-10-28 for:【JHHB-769】合并单据和任务处理为办理
  showSubmitBtn: {
    type: Boolean,
    default: true,
    required: true,
  },
  // update-end-author:liaozhiyang date:2025-10-28 for:【JHHB-769】合并单据和任务处理为办理
})

// 定义 emits
const emits = defineEmits(['back', 'success', 'formSuccess'])
const selectorId = uuid()
const toast = useToast(selectorId)
// 定义响应式数据
const onlinekey = ref(1)
// 刷新状态
const reFresh = ref(false)
// 子表刷新
const subReload = ref(true)
// 请求地址
const url = ref({
...
onMounted(() => {
  ...
  // update-begin-author:liaozhiyang date:2025-10-28 for:【JHHB-769】合并单据和任务处理为办理
  uni.$on('onlineLoader:formSubmit', (showMessage: boolean = true) => {
    formSubmit(showMessage)
  })
  uni.$on('onlineLoader:validateForm', (callback: Function) => {
    fieldCheck(formData.value).then((res) => {
      // 取反跟表单设计器一致，true表示校验通过，false表示校验失败
      callback(!res);
    })
  })
  // update-end-author:liaozhiyang date:2025-10-28 for:【JHHB-769】合并单据和任务处理为办理
...
const getDateExtendType = (formSchema: any) => {
  if (formSchema.fieldExtendJson) {
    const fieldExtendJson = JSON.parse(formSchema.fieldExtendJson)
    const mapField = {
      month: 'year-month',
      year: 'year',
      quarter: 'quarter',
...
/** 统一 Online 日期字段的回显和提交格式。 */
function normalizeDateFieldValue(value: any, fieldType: string) {
  if (!value) {
    return value
  }
  if (isNumber(value)) {
    return formatDate(value, fieldType === 'datetime' ? 'yyyy-MM-dd HH:mm:ss' : 'yyyy-MM-dd')
  }
  if (fieldType === 'datetime' && isString(value)) {
...
/** 格式化表单中的日期字段。 */
function normalizeFormDateFields(data: any) {
  rootProperties.value
    .filter((item) => item.type === 'date' || item.type === 'datetime')
    .forEach((item) => {
      data[item.key] = normalizeDateFieldValue(data[item.key], item.type)
    })
  return data
}
...
function handleMultiOrDateField() {
  ...
  // 日期字段
  const dateFieldArr = rootProperties.value.filter(
  ...
  finalData = Object.keys(finalData).reduce((acc, key) => {
    ...
    if (dateFieldArr.length > 0) {
      ...
      if (dateField) {
        value = normalizeDateFieldValue(value, dateField.type)
      }
    }
    acc[key] = value
    return acc
 ...
 * @param e
 */
async function formSubmit(showMessage: boolean = true) {
  console.log('表单提交事件', formData.value)
  // 防抖处理：如果正在提交中，直接返回
  if (isSubmitting.value) {
    console.log('表单提交防抖：正在提交中，忽略本次请求')
    return
  }
  // 设置提交状态
  ...
  try {
    setTimeout(async () => {
      ...
      // 处理特殊字段
      const finalData = await handleMultiOrDateField()
      // 主子表的增加子表的校验和数据拉取
      if (!single.value && subTable.value.length) {
        ...
        const promises = subTable.value.map(async (sub) => {
          if (dyRefs.value[sub.key]) {
            ...
            const subData = await dyRefs.value[sub.key].beforeSubmit()
      ...
      if (props.flowEdit === true) {
        ...
        // 表单编辑
        await handleEdit(finalData, showMessage)
      } else {
        // 表单新增
    ...
    if (pattern) {
      if (pattern == 'only') {
        ...
        if (values[item.key] && !regex.test(values[item.key])) {
          const errorInfo = item?.formSchema?.errorInfo || '格式不正确!'
 ...
 * 编辑保存
 */
const handleEdit = (finalData: any, showMessage: boolean = true) => {
  loading.value = true
  http
    ...
    .then((res: any) => {
      ...
      isSubmitting.value = false
      if (res.success) {
        emits('success', formData.value.id, showMessage)
      } else {
        toast.warning(res.message)
      }
    })
 ...
 * 设置子表字段值
 */
function setSubTableFieldsValue() {
  if (subTable.value.length) {
    subTable.value.forEach((sub) => {
      if (props.defaultFormData[sub.key]) {
        formData.value[sub.key] = props.defaultFormData[sub.key].map((item: any) => {
          return omit(item, ['create_by', 'create_time', 'del_flag', 'sys_org_code', 'id'])
        })
      }
    })
    subReload.value = false
    nextTick(() => {
      subReload.value = true
    })
  }
}
/**
...
const loadByTableName = (dataID: any, table?: string) => {
  ...
  // #endif
  const urlStr = url.value.load + (props.table || table)
  http.get(urlStr, { taskId: props.taskId }).then(async (res: any) => {
    ...
    if (res.success) {
      ...
      if (props.edit === true || props.flowEdit === true) {
        ...
        // 新增页面处理表单默认值
        if (props.defaultFormData && props.izCopy === true) {
          formData.value = { ...props.defaultFormData }
          setSubTableFieldsValue()
        } else {
  ...
  Object.keys(properties).map((key) => {
    if (key) {
      ...
      if (item.view === 'tab') {
        ...
        formData.value[key] = ''
        if (key != 'bpm_status') {
          const fp = FormProperty(key, item, formSchema.required)
          rootProps.push(fp)
        }
      }
    }
...
const loadFormData = async () => {
  ...
  if (res.success) {
    formData.value = normalizeFormDateFields({ ...res.result })
    if (single.value) {
      // 过滤单表的富文本和markdown标签
...
onBeforeUnmount(() => {
  // update-begin-author:liaozhiyang date:2025-10-28 for:【JHHB-769】合并单据和任务处理为办理
  uni.$off('onlineLoader:formSubmit')
  // update-end-author:liaozhiyang date:2025-10-28 for:【JHHB-769】合并单据和任务处理为办理
})
onUnmounted(() => {
  uni.hideLoading()
