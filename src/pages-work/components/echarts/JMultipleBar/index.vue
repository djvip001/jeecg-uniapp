...
// 图表数据查询
const [{ dataSource, reload, pageTips, config }, { queryData }] = useChartHook(props, initOption)
...
function initOption(data) {
  ...
  if (chartData && chartData.length > 0) {
    ...
    const dataset = handleDataSet(chartData)
    ...
    dataset.dimensions.forEach((series, index) => {
      if (index > 0) {
        ...
        if (configOption?.series && configOption?.series?.length > 0) {
          const optSeries = configOption.series[0]
          legengColor = configOption.series[0]?.color
...
function handleDataSet(chartData) {
  const dataObj = { dimensions: [], source: [], average: 0 }
  const dataList = []
  // 获取系列
  // @ts-ignore
  const dimensions = ['stack', ...new Set(chartData.map((item) => item.type))]
  // 获取name集合
  // @ts-ignore
  ...
  nameArr.forEach((name) => {
    // 筛选出指定name的对象集合
    const arr = chartData.filter((item) => item.name == name)
    // 获取对象集合的value
  ...
  const allValue = chartData.filter((chart) => chart.value > 0).map((item) => item.value)
