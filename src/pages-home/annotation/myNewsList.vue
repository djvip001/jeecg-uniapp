        ...
        <template v-for="(item, index) in dataList" :key="index">
          <wd-swipe-action>
            <view class="list bg-white" @click="showDetail(item)">
              <view v-if="item.izTop" class="izTop">置顶</view>
              <view class="cIcon">
                <view
...
import MyNewsConditionFilter from './components/MyNewsConditionFilter.vue'
...
const getParams = ({ pageNo, pageSize }) => {
  ...
  if (msgCategory.value) {
    if (['1', '2'].includes(msgCategory.value)) {
      result.msgCategory = msgCategory.value
    } else {
      result.noticeType = msgCategory.value
    }
  } else if (noticeType.value) {
    result.noticeType = noticeType.value
  }
  if (conditionStartDate.value) {
    result.sendTimeBegin = dayjs(conditionStartDate.value).format('YYYY-MM-DD')
  }
  if (conditionEndDate.value) {
    result.sendTimeEnd = dayjs(conditionEndDate.value).format('YYYY-MM-DD')
  }
  return result
}
...
const queryList = (pageNo, pageSize) => {
  ...
  http
    .get('/sys/sysAnnouncementSend/getMyAnnouncementSend', { ...params })
    .then((res: any) => {
      if (res.success) {
...
const showDetail = (record) => {
  if (record.msgAbstract) {
    try {
      ...
      if (json.taskDetail) {
        goBpmDetail({ ...record, successed: 1 })
        return
      }
    } catch (e) {
      console.error('getHrefText:msgAbstract参数不是JSON格式')
    }
  }
  if (record.busType == 'email') {
    ...
    goBpmList(record.busId)
  } else if (record.busType == 'bpm_cc') {
    goBpmDetail(record)
  } else {
    paramsStore.setPageParams('annotationDetail', {
      data: record,
      routeMethod: 'replace',
      backRouteName: 'myNewsList',
    })
    router.push({
...
const goEmailDetailPage = (item) => {
  ...
  uni.navigateTo({
    url: '/pages-super/mail/mailDetail?id=' + item.busId + '&routerName=myNewsList',
  })
}
// 去处理流程列表
  ...
  http.get(url, params).then((res: any) => {
    ...
    if (data.taskIsHandel == true) {
      ...
      const params1 = {
        ...
        routerName: 'myNewsList',
      }
      console.log('goBpmList***********params>>', params1)
      paramsStore.setPageParams('myTaskDetail', {
        data: params1,
        routeMethod: 'replace',
        routeName: 'myNewsList',
      })
      router.push({
        name: 'myTaskDetail',
      })
...
// 去流程详情页面
const goBpmDetail = (record) => {
  let taskInfo = JSON.parse(record.msgAbstract)
  console.log('goBpmDetail>>taskInfo>>', taskInfo)
  let params = {
    instanceId: taskInfo.procInsId,
    successed: record.successed == 1 ? 1 : 0,
    routerName: 'myNewsList',
    taskDefKey: taskInfo.taskDefKey,
    taskId: taskInfo.taskId,
  }
  paramsStore.setPageParams('taskDetail', {
    data: params,
  })
  router.push({
    name: 'taskDetail',
...
const handleFilterChange = (filters) => {
  titile.value = filters.titile
  sender.value = filters.sender
  msgCategory.value = filters.msgCategory
  noticeType.value = ''
  conditionStartDate.value = filters.startTime
  conditionEndDate.value = filters.endTime
  paging.value.reload()
}
// 收藏与取消收藏
...
onLoad((options) => {
  ...
  if (options?.msgCategory) {
    msgCategory.value = options.msgCategory
  }
  if (options?.noticeType) {
    noticeType.value = options.noticeType
